#include <iostream>
#include "L1Trigger/Phase2L1ParticleFlow/interface/TauNNIdHW.h"

#include "L1Trigger/Phase2L1ParticleFlow/src/nnet_utils/nnet_layer.h"
#include "L1Trigger/Phase2L1ParticleFlow/src/nnet_utils/nnet_activation.h"

//hls-fpga-machine-learning insert weights
#include "L1Trigger/Phase2L1ParticleFlow/data/weights/w1.h"
#include "L1Trigger/Phase2L1ParticleFlow/data/weights/b1.h"
#include "L1Trigger/Phase2L1ParticleFlow/data/weights/w2.h"
#include "L1Trigger/Phase2L1ParticleFlow/data/weights/b2.h"
#include "L1Trigger/Phase2L1ParticleFlow/data/weights/w3.h"
#include "L1Trigger/Phase2L1ParticleFlow/data/weights/b3.h"
#include "L1Trigger/Phase2L1ParticleFlow/data/weights/w4.h"
#include "L1Trigger/Phase2L1ParticleFlow/data/weights/b4.h"

TauNNIdHW::TauNNIdHW() { NNvectorVar_.clear(); }
TauNNIdHW::~TauNNIdHW(){ }

void TauNNIdHW::initialize(const std::string &iInput, const std::string &iWeightFile, int iNParticles) {
...
result_t TauNNIdHW::EvaluateNN() {
    ...
    //for(unsigned i0 = 0; i0 < NNvectorVar_.size(); i0++) { 
    //  std::cout << " ---> " << i0 << " -- " << NNvectorVar_[i0] << " -- " << data[i0]  << std::endl;
    //}
    //std::cout << " -----------------------------> " << std::endl;
    ...
    for(int i0 = 0; i0 < N_LAYER_2; i0++) { 
      std::cout << "---> " << layer2_out[i0] << std::endl;
    }
    */
...
result_t TauNNIdHW::compute(l1t::PFCandidate &iSeed, std::vector<edm::Ptr<l1t::PFCandidate>> &iParts) {
  for (unsigned i0 = 0; i0 < fNParticles_; i0++) {
    fPt_.get()[i0]  = 0.;
    fEta_.get()[i0] = 0.;
    fPhi_.get()[i0] = 0.;
  ...
  std::sort(iParts.begin(), iParts.end(), [](edm::Ptr<l1t::PFCandidate> i, edm::Ptr<l1t::PFCandidate> j) { return (pt_t(i->pt()) > pt_t(j->pt())); });  
  for (unsigned int i0 = 0; i0 < iParts.size(); i0++) {
    if (i0 > fNParticles_) break;
    ...
    if (lDPhi > lMPI)  lDPhi  = lDPhi-lMPI;
    if (lDPhi < -lMPI) lDPhi  = lDPhi+lMPI;
    fPhi_.get()[i0]           = lDPhi;
